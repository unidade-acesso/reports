# Criar repositório para relatório de auditoria
...
Vamos criar um repositório com base num repositório "template". Desta forma, o ficheiro README - uma espécie de folha de rosto do repositório - fica criado, a estrutura de pastas também e, mais importante, as etiquetas que vão ser usadas para etiquetar cada issue ficam prontas a usar.
...
<https://github.com/unidade-acesso?tab=repositories>
...
```
https://github.com/unidade-acesso/report_099
```

 e vamos por começar por alterar a página de rosto do nosso repositório. Geralmente a página de rosto dos repositórios Git é conhecida pelo ficheiro [README.md](https://README.md). Atualmente temos nesta página uma espécie de template que foi copiada do repositório que serve de base à construção do presente repositório. 
 
 Para alterar o ficheiro [README.md](https://README.md) vamos usar o botão “edit file”.
 
![Figura 3: editar o ficheiro de rosto do repositório, o README.md](criarrepo_img/edit_readme.png)

A extensão ".md" indica que se trata de um ficheiro escrito em Markdown. 

O README encontra-se dividido em duas partes essenciais: 
- os metadados, ou variáveis, que contém todas as variáveis que vamos  usar no nosso ficheiro, e;
- a informação que vai ficar pública na página e que servirá de folha de rosto do nosso repositório.

A lógica de concentrar todas as variáveis, ou campos de edição numa só zona - nos metadados - visa facilitar o preenchimento. Significa isto que preenchido esta zona dos metadados, a nossa página está pronta. Ou melhor, quase pronta :-). Há ainda um pequeno ajuste a fazer.

Mas comecemos pelos metadados, variáveis que estão entre as linhas delimitadas pelos caracteres "---". Técnicamente esta zona dos metadados é também designada por front-matter do ficheiro.
...
Agora que preenchemos os 6 campos com a informação do sítio Web, da entidade, etc, podemos "gravar" o ficheiro, no que na linguagem do Git é efetuar "Commit". É, mais uma vez, uma botão que se encontra no canto superior da viewport do nosso ficheiro README.

![Figura 4: commit do README, ou seja grava o ficheiro README.md](criarrepo_img/commit_readme.png)

E o ficheiro [README.md](https://README.md) está pronto. Mais tarde, depois de registarmos os issues, de os etiquetar e de gerar o relatório através do script GitReport, vamos voltar a esta página para efetuar o upload do relatório.
...
Ainda na página do repositório, por exemplo do report_099, vamos à opção "Settings" e, no menu secundário, escolhemos a opção "Pages". Isto leva-nos à página "GitHub Pages". Aqui vamos à secção "Build and deployment". Nesta secção vamos escolher "main" e usamos o botão "save".
...
Agora, no menu principal, podemos voltar a selecionar a opção "Code" e aqui vamos abrir a secção "About", uma das primeiras secções existentes na coluna do lado direito.
...
<https://unidade-acesso.github.io/report_099/README>

## 5º passo: Registar o Relatório no índice de relatórios

No ficheiro [README.md](https://README.md) do repositório "Reports" mantemos um indice dos repositórios produzidos ou em produção. Se a equipa repeitar sempre este passo quando gera um repositório, vai ser possível encontrar qualquer relatório produzido ou em produção de forma rápida. Podemos usar a pesquisa do browser para pesquisar pelo nome do website.

Para o efeito, vamos ao repositório "reports":

<https://unidade-acesso.github.io/reports/>

E editamos o [README.md](https://README.md) deste repositório para acrescentar o nosso relatório. 
...
<https://github.com/unidade-acesso/report_099/issues/>

(...)

## 7º passo: Gerar o relatório via script GitReport

O gerador de relatórios GitReports encontra-se em:

http://10.55.37.17/gitreports/v2/reporte.html

(...)

## 8º passo: Efetuar upload do relatório e notificar o "cliente"

O upload do relatório faz-se de novo no endereço do repositório onde está o [README.md](https://README.md).

<https://github.com/unidade-acesso/report_099/>

O upload passa sempre por publicar 2 cópias do relatório que foi retirado do gerador GitReports.

Imaginemos que hoje é 01/04/2030 e que retirámos o ficheiro report.html do GitReports. Criemos então 2 ficheiros iguais com os nomes: 
...
Depois de carregados os dois ficheiros, eles vão surgir na diretoria do nosso repositório que se encontra acima do [README.md](https://README.md). 

Agora vamos fazer uma pequena atualização na nossa folha de rosto, ou seja do [README.md](https://README.md). 
